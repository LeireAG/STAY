...
```{r}
# load Garvan atlas
garvan <- readRDS("/your_path/miniatlas.rds")
merged <- readRDS("/your_path/vis_mod_nmf.rds")## your merged object with multiple samples 
...
ggsave(paste0("/your_path/", "map/", "garvan_major_map.jpg"), final_plot, height = 35, width = 25, dpi = 300, limitsize = FALSE)
```
...
# Merge non-HD sample plots into a final plot
final_plot <- patchwork::wrap_plots(sample_plots, ncol = 1)
ggsave(paste0("/your_path/", "map/", "garvan_minor_map.jpg"), final_plot, height = 35, width = 120, dpi = 300, limitsize = FALSE)
```
...
```{r}
features <- paste0("NMF_", 1:24)
ggsave(
    "/your_path/map/nmf.jpg",
...
pdf("/your_path/nmf_minor_celltype_correlation.pdf", width = 10, height = 10)
...
```{r}
saveRDS(merged, "/your_path/vis_mod_nmf_decon.rds")
```
