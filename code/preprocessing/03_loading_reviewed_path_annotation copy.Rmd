...
p_stroma <- ggplot(plot.df_stroma, aes(y = nCount_Spatial, x = patient, fill = method)) +
  #aes(y = log_nCount, x = patient, fill = method)) +
  geom_violin(trim = FALSE, scale = "width", width = 1) +
  geom_boxplot(width = 0.1, outlier.shape = NA, position = position_dodge(width = 1)) + labs(
    title = "Total UMI Count Distribution in Stromal Regions", x = "Patient", y = "Total UMI counts per spot") +
  #labs(title = "Log-transformed UMI Count Distribution in Stroma Layer", x = "Patient", y = "log10(UMI count + 1)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  #scale_y_continuous(limits = c(0, 7000)) +  # adjust this limit to match main figures
  scale_fill_manual(values = c("STAY" = "#FBB4AE",  # pastel red
                             "standard" = "#B3CDE3"))  # pastel blue
...
png(filename = "/my_path/stroma_comparisonraw.png", width = 10*png_res, height = 8*png_res, res = png_res);print(p_stroma);dev.off()
```
